...
public class SelectionnerLigue {
	public static void main(String[] args)
	{	
		JFrame frame = new JFrame(); //Créer une instance de JFrame  
		frame.setTitle("Sélectionner une ligue");
		frame.setSize(800, 500);
		frame.getContentPane().setBackground(Color.YELLOW);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
		JLabel label = new JLabel("Sélection ligue", JLabel.CENTER); //Créer le texte et le centrer
		label.setFont(new Font("Rockwell Nova", Font.PLAIN, 35));
		label.setVerticalAlignment(JLabel.TOP);
		label.setForeground(Color.RED);
		frame.add(label);
	
		JButton button = new JButton("Gérer les employés");//créer une instance de JButton  
		...
		JButton button2 = new JButton("Renommer");//créer une instance de JButton  
		...
		JButton button3 = new JButton("Supprimer");//créer une instance de JButton  
		button3.setBounds(440,280,300, 40);//x axes, y axes, largeur, hauteur  
		button3.setBackground(Color.GRAY); //Définir la couleur de fond  
		button3.setForeground(Color.WHITE);
		button3.setFont(new Font("Rockwell Nova", Font.PLAIN, 15));
		frame.add(button3);//Ajouter bouton dans JFrame  
        
		JButton button4 = new JButton("Retour");//créer une instance de JButton  
		button4.setBounds(515,350,150, 40);//x axes, y axes, largeur, hauteur  
		button4.setBackground(Color.GRAY); //Définir la couleur de fond  
